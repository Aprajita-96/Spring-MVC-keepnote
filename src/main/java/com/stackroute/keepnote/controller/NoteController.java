...
public class NoteController {
	...
	Note noteObject=(Note)context.getBean("note");
	NoteRepository noterepo=(NoteRepository)context.getBean("noteRepository");
	...
	@RequestMapping("/")
		...
		modelMap.addAttribute("info",noterepo.getAllNotes());
			return "index";
		}
	...
	*/
	...
	public String read(ModelMap map,@RequestParam("noteId") int id,@RequestParam("noteTitle") String title,@RequestParam("noteContent") String content,@RequestParam("noteStatus") String status) {
		Note note=new Note();
		note.setNoteId(id);
		...
		map.addAttribute("info",(noterepo.getAllNotes()));
	...
	*/
	...
	public String deleten(ModelMap model,@RequestParam("noteId") int noteId){
		noterepo.deleteNote(noteId);
		System.out.println("*********delete*********  "+ noteId);
		model.addAttribute("info",noterepo.getAllNotes());
		return "redirect:" +"/";
